...
Process the DNA sequence data, which has many points per species.
This combines the metadata dataframe and results in
one speck file for the sequence data. Labels and colors are generated
by the sequence_lineage.py script.
...
def process_data(datainfo, metadata):
    '''
    Process the sequence data. We combine the sequence data, the seq2taxon info, the metadata, and 
