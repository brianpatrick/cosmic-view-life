...
parser.add_argument("-i", "--input_dataset_json_file", help="Input dataset JSON file.", 
                    required=True)
parser.add_argument("-d", "--input_data_dir", help="Input directory for CSV files.",
                    required=True)
...
def make_points_asset_and_csv_from_dataframe(input_points_df, 
    ...
    with open(output_asset_filename, "w") as output_file:
        ...
        if colormapping.get("local_colormap", False):
            ...
            # This is a little different from the other output files, as it's not generated by this
            # script. Colormaps specified in the json file are pre-generated and just need to
            # be copied from the specified directory to the output asset dir. Note that colormaps
            # can live in their own directory - this allows us to change colormaps globally and 
            # the colors of everything changes consistently. At least, that's the idea.
            add_output_file(Path(args.input_data_dir) / colormapping['directory'] / colormapping['colormap'])
    ...
    if (rendered_labels):
        print("Making rendered labels assets... ", end="", flush=True)
...
def main():
    ...
    for row in input_dataset["datasets"]:
        ...
        # assume the file is in the input_data_dir.

        dataset_csv_filename = row["csv_file"]
        dataset_csv_fullpath = Path(args.input_data_dir) / Path(row.get("directory", "")) / Path(dataset_csv_filename)
        print("Reading file: " + str(dataset_csv_fullpath) + "... ", end="", flush=True)

        input_points_df = pd.read_csv(dataset_csv_fullpath)
